...
This builds both `exchange` and `exchange-mcp` and installs them to `~/.local/bin/`.
...
```bash
claude mcp add exchange -- exchange-mcp
```
...
This CLI ships with an MCP server for direct agent integration. See [Installation](#installation) for setup.

For agents that don't support MCP, copy the reference from [agent-instructions.md](agent-instructions.md)
